...
description: >-
 ...
 when a builder has a per-unit value and wants a total market magnitude they can defend line by
...
The smallest countable unit of value: one permit reviewed, one report, one plant shift, one invoice reconciled. It should be the same unit `physics-floor-gap` scored. If the builder cannot name it, stop and ask for it. Do not invent one.
...
## Examples
- [examples/sample.md](examples/sample.md) — Halcyon Safety's per-permit gas-safety review, sized bottom-up: 300 installations × 300 reviews/yr × £15 → £1.35M reachable revenue, weakest rung Thin, capture rung Fiction, and a top-down £2.1B TAM that reconciles Broken (0.013×) because it bundles hardware and consultancy the per-permit slice never touches. All numbers illustrative.
