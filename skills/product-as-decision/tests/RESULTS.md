...
|---|---|---|---|---|---|---|---|
| 01 Foundry Signal (12×, L0→L1) | 5 | 5 | 5 | 5 | 5 | 25 | Y |
| 02 Halcyon Safety hot-work (autonomy capped L2 by liability) | 5 | 5 | 5 | 5 | 5 | 25 | Y |
| 03 Meridian Grid SLA data-rights (~12×, L0→L2) | 5 | 5 | 5 | 5 | 5 | 25 | Y |
| 04 Larder Labs ticket-triage (autonomy axis, earned-rung) | 5 | 5 | 5 | 5 | 5 | 25 | Y |
...
- **The "after" value is frequently absent from the builder's input** (Golden 01 gives no 30 s; Golden 05 gives no 2 min), yet the Method needs an after to compute the compression ratio. The skill handles this correctly via mandatory `[Hypothesis]` tagging, and it does NOT trip the "fabricated a number" auto_fail (that guard targets asserted facts/citations, not a target the build must earn). Still, template Step 4 should say explicitly: **the after target comes from the builder's own claim ("we think we can cut it to X"); where the builder gives none, label it an illustrative/assumed target, do not silently mint a specific figure.** Otherwise a careless builder can misread a minted after (30 s, 2 min) as a commitment rather than an unproven hypothesis. Minor; does not block graduation.
...
## Refine run 2 — applied judge fixes: template Step 4 and Method Step 4 now state the after value comes from the builder's own stated target; where absent, it is labelled an illustrative `[Hypothesis]`, never a silently minted figure.
