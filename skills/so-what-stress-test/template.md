...
| 3 | What's the wedge? | Smallest forced action + cost of not adopting | <answer> | <rung> | pass / weak |
| 4 | What's the moat? | One asset that compounds with use + proof it has started | <answer> | <rung> | pass / weak |
| 5 | What kills it? | Named credible killer + early signal + a mitigation | <answer> | <rung> | pass / weak / fatal |

Ladder: money 1.0 · behaviour 0.7 · artefact 0.5 · verbal 0.3 · opinion 0.1.
Pass bar for Q1–Q4 is ≥ 0.7. Q5 passes only when the killer is named, has an early
signal, and has a credible mitigation.

## Verdict

Apply in order: any `fatal` → **KILL** · all five `pass` → **PASS** · one or more
`weak`, none `fatal` → **REDESIGN**.

> **Verdict: <PASS / REDESIGN / KILL>**
> One line why: <the binding constraint, named>

**Weakest question:** Q<n> — <question> (rung <x>, grade <weak/fatal>)

## Rewrite (fill only if REDESIGN)

The idea rewritten so Q<n> would now grade `pass`. Change the idea, not the words.
...
## Kill note (fill only if KILL)

> **Killer:** <the credible fatal>
> **Early signal already firing:** <what you can see today>
> **Why no mitigation clears it:** <one line>
> **The one condition that would reopen it:** <if any; else "none">
