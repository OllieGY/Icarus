...
name: lessons
description: >-
  Reads and writes the hub's append-only lessons log — what the studio learned the hard
  ...
  at the end of any fellow session worth remembering. Also fires BEFORE product work to
...
**Read first.** Before any Icarus stage runs on a fellow's problem, scan the log for an
...
for the studio to do it differently next time. Three tests:

| Test | Fails if |
|---|---|
| **Specific** | It names a fellow, a date, a number. "Communicate better" is not a lesson. |
...
1. Ask what happened, in the fellow's own words. Do not paraphrase it into studio jargon yet.
...
> **2026-03-11 — Mentix pilot scoped from a verbal yes.** Ran three weeks of build on
