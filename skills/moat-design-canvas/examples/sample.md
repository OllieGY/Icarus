...
# Worked example — Meridian Grid's cross-operator data-centre risk benchmark

Meridian Grid builds infrastructure-risk products for data-centre operators. This is a test fixture: numbers and dates are illustrative, not real client data. "Now" is 2026-07.
...
- Pilot #1 contracts do NOT yet contain a data-rights clause; Meridian Grid is using the incident data on a handshake. [Fact]
...
|---|---|---|---|---|
| 1 Wedge | 2026-07 [Fact] | Passed 5/5 | 3 of 4 operators changed a maintenance decision on the report. Behaviour 0.7 [Fact] | Each run leaves that operator's incident history in Meridian Grid |
...
| 3 Lock-in | 2027-03 [Assumption] | Enough risk-trend history to be relied on externally | An operator cites its Meridian Grid risk-trend in an insurer submission or board pack. Artefact 0.5 [Hypothesis] | Switching cost = quarters of trend history + the peer baseline you lose on exit |
...
Note on Gate 4: it currently **cannot clear** — the data-rights clause is unsigned (see §5). The corpus is not yet contractually Meridian Grid's, so a competitor could accumulate the same asset from the same operators. Routed to `data-rights-clause`.

## 2. The incumbent-veto sentence

**Most-likely incumbent to copy (X):** the third-party DCIM / monitoring platform Meridian Grid's operators already run.
...
|---|---|---|
| Structural, not speed | Names the incumbent's own structure | PASS — it names their data-isolation contracts and their trust proposition, not Meridian Grid's effort |
...
**Veto category:** Data rights + channel conflict. The corpus Meridian Grid can pool is the corpus the incumbent contractually cannot. This is exactly why Gate 4's signed clause matters: the veto only holds if Meridian Grid's own right to pool is real (→ `data-rights-clause`).
...
| 2 | Lock-in | Zero operators have referenced their Meridian Grid risk-trend in an external document (insurer / board / audit) by 2027-03 | Monthly | Switching cost is imagined; lock-in gate failing |
...
- Put the three falsifiers on a monthly review starting now. The moat is a claim Meridian Grid checks, not a slide it shows once.
