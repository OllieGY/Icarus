---
description: Route a builder or a product through the Icarus stages — which apply, which to skip, and in what order.
argument-hint: "[situation — where you are, what exists, what evidence you hold]"
allowed-tools: Read Glob Grep Write Skill
---

Run the `start-here-router` skill on this situation:

$ARGUMENTS

Before routing, read `${CLAUDE_PLUGIN_ROOT}/GOLDEN_RULES.md`. Classify to the highest rung of evidence they can
actually show, and round down — a builder who says "mature product" but cannot show
flattening retention is Type B, not C, and you say so.

Return the stage ledger (KEEP / TRIM / SUBTRACT / LEAP across the eleven sections) and the
think:build:test ratio tied to that type's dominant failure mode. Do not grade the idea —
that is `concept-council` or `/stress-test`. If there is no product signal and no problem
signal, ask the one classifying question and stop.
